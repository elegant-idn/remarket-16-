...
class SendLinks extends Component {
  ...
  sendCouponForm = (e) => {
    ...
    if (name.includes("linkByEmail")) {
      data = new FormData(
        document.querySelector(`form[id='linkByEmail-${this.props.id}']`)
      );
      type = "email";
      text = document.querySelector(".couponEmail").innerHTML;
    } else {
      data = new FormData(
        document.querySelector(`form[id='linkBySms-${this.props.id}']`)
      );
      type = "phone";
    ...
    if (type == "email") {
      form = document.querySelector(
        `form[name='linkByEmail-${this.props.id}']`
      );
    } else {
        ...
        {isSellDeadline ? (
          <div className="row countdownLinks">
            ...
            <div className="col-md-6 text-center">
              ...
              >
                Angebot jetzt per E-Mail sichern
              </button>
              <div className="form">
                <form
                  id={`linkByEmail-${id}`}
                  name={`linkByEmail-${id}`}
                  onSubmit={this.sendCouponForm}
                >
                  <input
                    type="email"
                    name="email"
                  ...
                  <div className="textarea couponEmail">
                    Hallo,
                    <br />
                    gerne senden wir Ihnen unser Angebot mit dem +20.00 CHF
                    Gutschein, Link: <br />
                    <a
                      contentEditable={false}
                      href={
                        link + "?shortcode=" + this.props.sellCouponShortcode
                      }
                    >
                      {link + "?shortcode=" + this.props.sellCouponShortcode}
                    </a>
                  </div>
                  <button className="btn" onSubmit={this.sendCouponForm}>
                    Senden
                  </button>
                </form>
              </div>
            ...
            <div className="col-md-6 text-center">
              <button className="btn" onClick={() => this.clickButton("sms")}>
                Angebot jetzt per SMS sichern
              </button>
              <div className="form">
                <form
                  id={`linkBySms-${id}`}
              ...
              <span className="itemBtn">
                <label htmlFor={`email-${id}`}>
                  {isAssistantQuestion ? (
                    <img
                      loading="lazy"
                      src="/images/design/mail-links-white.svg"
                      alt=""
                    />
                  ) : (
                    <img
                      loading="lazy"
                      src="/images/design/mail-links-black.svg"
                      className="mail-links-img"
                      alt=""
                    />
                  )}
                </label>
              ...
              <span className="itemBtn">
                <label htmlFor={`sms-${id}`}>
                  {isAssistantQuestion ? (
                    <img
                      loading="lazy"
                      src="/images/design/sms-link-white.svg"
                      alt=""
                    />
                  ) : (
                    <img
                      loading="lazy"
                      src="/images/design/sms-link-black.svg"
                      className="sms-links-img"
                      alt=""
                    />
                  )}
                </label>
              ...
              <span className="itemBtn">
                <label htmlFor={`clipboard-${id}`}>
                  {isAssistantQuestion ? (
                    <img
                      loading="lazy"
                      src="/images/design/copy-to-clipboard-white.svg"
                      alt=""
                    />
                  ) : (
                    <img
                      loading="lazy"
                      src="/images/design/copy-to-clipboard-black.svg"
                      className="copy-to-clipboard-img"
                      alt=""
                    />
                  )}
                </label>
